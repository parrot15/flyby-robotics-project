...
LidarClient::LidarClient(
    ...
    const ouster::sensor::sensor_info& meta, double distance_threshold)
    : handle_(handle),
      meta_(meta),
...
void LidarClient::process_scan(int packet_buffer_size, int points_per_batch) {
  ...
  std::cout << "Sent number of points (" << num_points << ") to lidar receiver."
            << std::endl;
...
void LidarClient::send_num_points(size_t num_points) {
  json num_points_msg = {{"num_points", num_points}};
...
void LidarClient::process_coords(const ouster::LidarScan& scan,
                                 int points_per_batch) {
  // Extract coordinates and convert them to cartesian form.
  ouster::XYZLut lut = ouster::make_xyz_lut(meta_);
  auto coords =
      ouster::cartesian(scan.field(ouster::sensor::ChanField::RANGE), lut);
  ...
  for (int i = 0; i < total_points; i += points_per_batch) {
    ...
    json batch_json = {{"frameId", scan.frame_id},
