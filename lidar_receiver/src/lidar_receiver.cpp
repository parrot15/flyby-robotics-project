...
LidarReceiver::LidarReceiver() : is_connected_(false), num_points_(0) {}

LidarReceiver::~LidarReceiver() { this->stop(); }

void LidarReceiver::listen(const std::string& host, int port) {
  acceptor_ = std::make_unique<tcp::acceptor>(
      ioc_, tcp::endpoint{net::ip::make_address(host),
...
void LidarReceiver::accept() {
  ...
  ws_ =
      std::make_unique<websocket::stream<tcp::socket>>(std::move(*tcp_socket_));
  ws_->accept();
  is_connected_ = true;
  std::cout << "Successfully connected to lidar client." << std::endl;
}

void LidarReceiver::start_visualization(
    const std::string& visualization_title) {
  if (!is_connected_) {
    return;
  }
  ...
  message_thread_ =
      std::thread([this, cloud]() { this->message_handler(cloud); });
...
void LidarReceiver::receive_num_points() {
  ...
  std::cout << "Received number of points (" << num_points_
            << ") from lidar client." << std::endl;
}

void LidarReceiver::message_handler(
    std::shared_ptr<ouster::viz::Cloud> cloud) const {
  try {
